// https://github.com/ocaml/ocaml/blob/01c6f16cc69ce1d8cf157e66d5702fadaa18d247/stdlib/seq.ml
...
@inline let iteri = (xs, f) => iteriAux(xs, 0, f)
let forEachi = iteri
...
@inline let foldLefti = (xs, accu, f) => foldLeftiAux(xs, accu, 0, f)

let reduceRighti = foldLefti
...
let toArray = xs => {
  let arr = []
  xs->iter(x => arr->Js.Array2.push(x)->ignore)
  arr
}
